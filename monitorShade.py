...
Author: dforcade / rundowntown

Description:
------------
This is the main script for the Screen Brightness Control GUI application using PySide6.
The application allows users to:

- Control the brightness of up to 8 monitors individually or all at once
- Switch between Auto Mode (single brightness) and Toggle Mode (alternate brightness levels)
- Create, save, load, and delete brightness profiles with monitor selections, brightness, dimness, overlay opacity, and hotkey bindings
- Adjust screen overlay opacity (software darkening) for each monitor
- Assign and trigger hotkeys to toggle between brightness states in Toggle Mode
- View dynamic monitor icons and brightness frames with color-coded feedback

Core Features:
--------------
- PySide6 GUI layout using QMainWindow and nested layouts (QSplitter, QVBoxLayout, QHBoxLayout)
- Real-time brightness control via the `screen_brightness_control` module
- Custom overlay windows using semi-transparent QWidget overlays
- Profile persistence with JSON file (`profiles.json`)
- Manual and auto brightness modes
- Keyboard hotkey handling and configuration
- Icons, logos, and monitor imagery loaded from `assets/`

File Structure Dependencies:
----------------------------
- /assets/
    - monitorImage_on.png
    - monitorImage_off.png
    - monitor-icon.ico
    - logo_160w.png
- profiles.json (automatically generated)

Note:
-----
Designed for multi-monitor setups with a max of 8 monitors
...
from PySide6.QtGui import QPixmap, QKeySequence, QIcon
...
def resource_path(rel_path):
    """
    Get absolute path to resource, works in dev and for PyInstaller bundles.
    """
...
class ClickableFrame(QFrame):
    ...
    def set_image(self, image_name):
        full = resource_path(os.path.join("assets", image_name))
        pixmap = QPixmap(full)
...
class BrightnessControlApp(QMainWindow):
    def __init__(self):
        ...
        self.setWindowIcon(QIcon(resource_path(os.path.join("assets", "monitor-icon.ico")))) ## Set Window Taskbar Icon
    ...
    def create_left_side_menu(self):
        ...
        pixmap = QPixmap(resource_path(os.path.join("assets", "logo_160w.png")))
